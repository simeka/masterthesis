...
\foreach \i in {0,...,5}
\foreach \j in {0,...,0} \draw[-stealth,orange!100!white] (hiddenl\i) -- (output\j);
...
\draw (input1) ++ (-0.25,-0.20) node[red!100!white,above] {\tiny $x_0$};
\draw (hidden5) ++ (0.0,0.25) node[blue,above] {\tiny hidden (1)};
...
\node[color=green,draw,circle,inner sep=1.5pt] (bias2) at (2.25+1.5,5*0.5+0.5) {};
...
% weight labels
\draw (input0) ++ (0.5,-1.0) node[blue!100!white,above] {\tiny $\textbf{W}^{\text{(1)}}$};
\draw (hiddenl0) ++ (1.1,0) node[orange!100!white,above] {\tiny $\textbf{W}^{\text{(out)}}$};
