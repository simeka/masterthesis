...
if __name__ == "__main__":
    ...
    plt.legend(["$p_{%s}$" % (i) for i in range(1, 5)], loc='upper right', bbox_to_anchor=(1, 1))
    ...
    for p, (spiketrain, c) in pop_xor_spiketrains.items():
        if p != 3:
            plt.scatter(spiketrain[:, 1] * 1e6, spiketrain[:, 0], s=s[p], lw=lw, facecolors='none',
                        edgecolors=colors[p], label="$p_{%s}$" % (p+1))
        else:
            plt.scatter(spiketrain[:, 1] * 1e6, spiketrain[:, 0], s=s[p], lw=lw, color=colors[p], label="$p_{%s}$" % (p+1))
    ...
    #     plt.plot(dynamic_range_in_V, lin_fit(dynamic_range_in_V), label="linear fit %s " % title[quad]) #: %s *x + %s " %(np.round(p[0],2), np.round(p[1],2)))
